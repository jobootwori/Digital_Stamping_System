...
export function Upload() {
  ...
  // State for QR code and serial number
  ...
  // State for serial number positioning
  ...
  const addQrCode = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setQrCode(`http://127.0.0.1:3033/verify/${response.data.serial_number}`);
      } else {
  ...
  // Convert SVG QR Code to Image when QR Code Changes
  useEffect(() => {
    if (!qrCode) return;
    ...
    const convertSvgToImage = () => {
      ...
      img.onload = () => {
        setQrImage(img);
        URL.revokeObjectURL(svgUrl); // Clean up memory
      };
      img.src = svgUrl;
    };

    convertSvgToImage();
  }, [qrCode]); // Run whenever qrCode changes
    ...
    return (
      ...
      >
        <Rect width={qrSize} height={qrSize} fill="white" shadowBlur={5} />
         ...
         <KonvaImage image={qrImage} width={qrSize} height={qrSize} />{/* <QRCodeSVG value={qrCode} size={qrSize} /> */}
  ...
  const addStampToCanvas = (stamp) => {
    const newStamp = {
      ...
      shape: stamp.shape,
      size: parseInt(stamp.size, 10) || 50,
    };
  ...
  const renderStamps = () =>
    stamps.map((stamp) => {
      const fontSizeMain = (stamp.size || 50) * 0.3;
      const fontSizeSub = (stamp.size || 50) * 0.2;
      const totalTextHeight = fontSizeMain + fontSizeSub;
      return (
        <Group
          key={stamp.id}
          draggable
  ...
  const saveCanvasToDatabase = async () => {
    try {
      ...
      if (fileType === 'image') {
        ...
        const uri = stageRef.current.toDataURL({ pixelRatio: 2 });
        console.log('Generated Data URL:', uri.slice(0, 50));
    ...
    <Box sx={{ textAlign: 'center', p: 4 }}>
      ...
      {serialNumber && (
        <Box sx={{ mt: 3 }}>
          ...
          <QRCodeSVG value={`http://127.0.0.1:3033/verify/${serialNumber}`} size={150} />
        </Box>
      )}
...
// import axios from 'axios';
// import { QRCode } from 'qrcode.react'; // <-- Updated: default import
// import React, { useState, useRef, useEffect } from 'react';
// import {
//   Stage,
//   Layer,
...
//       } else if (
//         fileType === 'pdf' ||
//         (file && file.type === 'application/x-pdf') ||
...
//             </Box>
//           ))}
//           {/* Render QR Code and Serial Number in Library (if needed) */}
//           {serialNumber && (
//             <Box sx={{ mt: 3 }}>
//               <Typography variant="h6">Serial Number: {serialNumber}</Typography>
//               <QRCode value={`https://your-domain.com/verify/${serialNumber}`} size={150} />
